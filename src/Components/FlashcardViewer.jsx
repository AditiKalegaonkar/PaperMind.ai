...
const FlashcardViewer = ({ flashcards }) => {
  ...
  useEffect(() => {
    if (typeof flashcards === 'string') {
      try {
        ...
        if (Array.isArray(parsed)) { setParsedCards(parsed); return; }
      } catch (_) {}
      const m = flashcards.match(/\[[\s\S]*\]/);
      if (m) {
        try { setParsedCards(JSON.parse(m[0])); return; } catch (_) {}
      }
      setParsedCards([]);
  ...
  const nextCard   = () => { setCurrentIndex(p => (p + 1) % parsedCards.length); setFlipped({}); };
  const prevCard   = () => { setCurrentIndex(p => (p - 1 + parsedCards.length) % parsedCards.length); setFlipped({}); };
    ...
    <div className="pm-flashcard-tabs">
      {['grid', 'list', 'quiz'].map(v => (
        <button
          ...
          onClick={() => { setView(v); setFlipped({}); setCurrentIndex(0); }}
        >
    ...
    <div className="pm-flashcard-viewer">
      ...
      <div className="pm-flashcard-grid">
        {parsedCards.map((card, i) => (
          <div
            key={i}
            className={`pm-flashcard-grid-item${flipped[i] ? ' flipped' : ''}`}
  ...
  return (
    <div className="pm-flashcard-viewer">
      <div className="pm-flashcard-header">
        <h3>Quiz ({currentIndex + 1} / {parsedCards.length})</h3>
        <Tabs />
      </div>
